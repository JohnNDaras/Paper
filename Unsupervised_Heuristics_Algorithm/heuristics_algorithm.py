...
class Heuristics_Algorithm:
    ...
    def getWeight(self,sourceId, tEntity) :
      ...
      if self.wScheme == 'CF':
        ...
        if(self.getNoOfBlocks(self.sourceData[sourceId].bounds) + self.getNoOfBlocks(tEntity.bounds) - commonBlocks == 0):
          return commonBlocks
        else:
          return commonBlocks / (self.getNoOfBlocks(self.sourceData[sourceId].bounds) + self.getNoOfBlocks(tEntity.bounds) - commonBlocks)
                    ...
                    if (self.validCandidate(candidateMatchId, targetGeom.envelope)):
                        ...
                        if len(self.sample) < self.SAMPLE_SIZE:
                              ...
                              if self.random_number == 0:
                                self.sample.append((candidateMatchId, targetId, targetGeom))

                                if self.relations.verifyRelations(candidateMatchId, targetId, self.sourceData[candidateMatchId], targetGeom, None, None , 0, 0):
                                  self.detectedQP += 1
    ...
    def verification(self):
        ...
        while(not self.topKPairs.empty()):
            ...
            if self.relations.verifyRelations(source_id, target_id, self.sourceData[source_id], tEntity, self.heuristicCondition, self.condition_limit , self.dynamic_factor, self.violation_limit) == 2:
